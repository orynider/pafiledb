...
* @version $Id: functions_pafiledb.php,v 1.67 2013/10/03 10:05:44 orynider Exp $
 ...
 *
 */
class pafiledb extends pafiledb\core\pafiledb_auth
{
	...
	var $this->auth_user = array();
	var $this->auth_global = array();	
	...
	* @param string 										$pa_category_table
	*
	*/
		...
		$pa_category_table)
	{
		...
		$this->auth 				= $auth;		
		$this->pa_category_table 	= $pa_category_table;
	...
	function init()
	{
		...
		for( $i = 0; $i < count( $cat_rowset ); $i++ )
		{
			if ( $this->auth_user[$cat_rowset[$i]['cat_id']]['auth_view'] )
			{
				...
				if ($this->comments[$cat_rowset[$i]['cat_id']]['activated'] && !$this->comments[$cat_rowset[$i]['cat_id']]['internal_comments'] && intval($this->comments[$cat_rowset[$i]['cat_id']]['comments_forum_id']) < 1)
				{
					message_die(GENERAL_ERROR, 'Init Failure, phpBB comments with no target forum_id :( <br> Category: ' . $cat_rowset[$i]['cat_name'] . ' Forum_id: ' . $this->comments[$cat_rowset[$i]['cat_id']]['comments_forum_id']);
				}
		...
		if ( isset( $this->subcat_rowset[$cat_id] ) )
		{
			...
			foreach( $this->subcat_rowset[$cat_id] as $subcat_id => $subcat_row )
			{
				...
				if ( !empty( $last_file_info['file_id'] ) && $this->auth_user[$subcat_id]['auth_read'] )
				{
					...
					$last_file .= '<a href="' . mx_append_sid( $this->this_mxurl( 'action=file&file_id=' . $last_file_info['file_id'] ) ) . '"><img src="' . $images['pa_icon_latest_reply'] . '" border="0" alt="' . $lang['View_latest_file'] . '" title="' . $lang['View_latest_file'] . '" /></a>';
				}
				else
				{
				...
				$template->assign_block_vars( 'catcol.no_cat_parent', array(
					...
					'CAT_IMAGE' => ( $is_new ) ? $images['pa_folder_new'] : $images['pa_folder'],
...
class pafiledb_public extends pafiledb
{
	...
	function module( $module_name )
	{
		if ( !class_exists( 'pafiledb_' . $module_name ) )
		{
			...
			require_once( $module_root_path . 'pafiledb/modules/pafiledb_' . $module_name . '.' . $phpEx );
	...
	{
		global $template, $pafiledb_functions;
