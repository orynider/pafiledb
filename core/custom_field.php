...
class custom_field
{
	...
	*/
		...
		$custom_data_table)
	{
		...
		{
			$functions->message_die( GENERAL_ERROR, 'Couldnt Query Custom field', '', __LINE__, __FILE__, $sql );
		}
			...
			if ( isset( $this->field_data_rowset[$file_id] ) )
			{
				...
				foreach( $this->field_data_rowset[$file_id] as $field_id => $data )
				{
					...
					else
					{
						...
						if ( !( $db->sql_query( $sql ) ) )
						{
							$functions->message_die( GENERAL_ERROR, 'Could not delete custom data', '', __LINE__, __FILE__, $sql );
						}
					}
				}
				return $message;
	...
	function update_add_field( $field_type, $field_id = false )
	{
		...
		if ( empty( $field_name ) )
		{
			$functions->message_die( GENERAL_ERROR, $lang['Missing_field'] );
		}
		...
		if ( !$field_id )
		{
			...
			if ( !( $db->sql_query( $sql ) ) )
			{
				$functions->message_die( GENERAL_ERROR, 'Could not add the new fields', '', __LINE__, __FILE__, $sql );
			}
			...
			if ( !( $db->sql_query( $sql ) ) )
			{
				$functions->message_die( GENERAL_ERROR, 'Could not set the order for the giving field', '', __LINE__, __FILE__, $sql );
			}
		}
		else
		{
			...
			if ( !( $db->sql_query( $sql ) ) )
			{
				$functions->message_die( GENERAL_ERROR, 'Could not update information for the giving field', '', __LINE__, __FILE__, $sql );
			}
		}
	}
	...
	function delete_field( $field_id )
	{
		...
		if ( !( $db->sql_query( $sql ) ) )
		{
			$functions->message_die( GENERAL_ERROR, 'Could not delete the selected field', '', __LINE__, __FILE__, $sql );
		}
	}
	...
	function file_update_data( $file_id )
	{
		...
		if ( !empty( $field ) )
		{
			foreach( $field as $field_id => $field_data )
			{
				...
				if ( !$db->sql_query( $sql ) )
				{
					$functions->message_die( GENERAL_ERROR, 'Could not delete data from custom data table', '', __LINE__, __FILE__, $sql );
				}
				...
				if ( !empty( $data ) )
				{
					...
					if ( !$db->sql_query( $sql ) )
					{
						$functions->message_die( GENERAL_ERROR, 'Could not add additional data', '', __LINE__, __FILE__, $sql );
					}
				}
			}
		}
