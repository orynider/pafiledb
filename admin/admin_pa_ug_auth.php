 ...
 * 
 *    $Id: admin_pa_ug_auth.php,v 1.1 2005/01/09 22:15:21 jonohlsson Exp $
 */
...
}elseif ( ( $mode == 'user' && ( isset( $_POST['username'] ) || $user_id ) ) || ( $mode == 'group' && $group_id ) )
{
	...
	for( $i = 0; $i < count( $cat_auth_fields ); $i++ )
	{
		...
		$pafiledb_template->assign_block_vars( 'acltype', array( 'L_UG_ACL_TYPE' => $cell_title ) 
			);
		$s_column_span++;
	}
	...
	if ( $mode == 'user' )
	{
		$pafiledb_template->assign_vars( array( 'USER' => true,
				'USERNAME' => $t_username,
	...
	{
		$pafiledb_template->assign_vars( array( 'USER' => false,
				'USERNAME' => $t_groupname,
				'GROUP_MEMBERSHIP' => $lang['Usergroup_members'] . ' : ' . $t_usergroup_list ) 
			);
	}

	$pafiledb_template->assign_vars( array( 'SHOW_MOD' => true,
...
}elseif ( ( $mode == 'global_user' && ( isset( $_POST['username'] ) || $user_id ) ) || ( $mode == 'global_group' && $group_id ) )
{
	...
	$pafiledb_template->assign_block_vars( 'cat_row', array( 'CAT_NAME' => ( $mode == 'global_user' ) ? $lang['User_Global_Permissions'] : $lang['Group_Global_Permissions'],
			'IS_HIGHER_CAT' => false,
			'PRE' => '',
	...
	for( $j = 0; $j < count( $global_auth_fields ); $j++ )
	{
		$pafiledb_template->assign_block_vars( 'cat_row.aclvalues', array( 'S_ACL_SELECT' => $optionlist_acl_adv[$j] ) 
			);
	}
	...
	$pafiledb_template->assign_vars( array( 'SHOW_MOD' => false,
...
{ 
	...
	$pafiledb_template->set_filenames( array( 'body' => ( $mode == 'user' || $mode == 'global_user' ) ? 'admin/user_select_body.tpl' : 'admin/auth_select_body.tpl' ) 
		);

	if ( $mode == 'user' || $mode == 'global_user' )
	{
		$pafiledb_template->assign_vars( array( 'L_FIND_USERNAME' => $lang['Find_username'],
	...
	else
	{
		...
		$pafiledb_template->assign_vars( array( 'S_AUTH_SELECT' => $select_list ) 
			);
	}
	...
	$pafiledb_template->assign_vars( array( 'L_' . $l_type . '_TITLE' => ( $mode == 'user' || $mode == 'global_user' ) ? $lang['Auth_Control_User'] : $lang['Auth_Control_Group'],
