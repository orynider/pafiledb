		...
		if ( $result = $db->sql_query( $sql ) )
		{
			...
			if ( $num_smilies )
			{
				...
				while ( list( $smile_url, $data ) = @each( $rowset ) )
				{
					...
					$pafiledb_template->assign_block_vars( 'smilies_row.smilies_col', array( 'SMILEY_CODE' => $data['code'],
				...
				if ( $mode == 'inline' && $num_smilies > $inline_rows * $inline_columns )
				{
					...
					$pafiledb_template->assign_vars( array( 'L_MORE_SMILIES' => $lang['More_emoticons'],
...
function pafiledb_page_header( $page_title )
{
	...
	$pafiledb_template->assign_vars( array( 'IS_AUTH_VIEWALL' => ( $pafiledb_config['settings_viewall'] ) ? ( ( $pafiledb->modules[$pafiledb->module_name]->auth_global['auth_viewall'] ) ? true : false ) : false,
...
function pafiledb_page_footer()
{
	...
	$pafiledb_template->assign_vars( array( 'JUMPMENU' => $pafiledb->modules[$pafiledb->module_name]->jumpmenu_option(),
			'L_JUMP' => $lang['jump'],
