...
class pafiledb_stats extends pafiledb_public
{
	function main( $action )
	{
		...
		foreach( $os_point as $index => $point )
		{
			...
			// MX
			$pafiledb_template->assign_block_vars( "downloads_os", array( 'OS_IMG' => "pafiledb/images/stats/" . $os_image[$index],
		...
		foreach( $agent_point as $index => $point )
		{
			...
			// MX
			$pafiledb_template->assign_block_vars( "downloads_b", array( 'B_IMG' => "pafiledb/images/stats/" . $agent_image[$index],
		...
		foreach( $os_point as $index => $point )
		{
			...
			$os_graphic = ( $os_graphic < $os_graphic_max - 1 ) ? $os_graphic + 1 : 0;
			// MX
			$pafiledb_template->assign_block_vars( "rating_os", array( 'OS_IMG' => "pafiledb/images/stats/" . $os_image[$index],
					'OS_NAME' => $os_lang[$index],
					'OS_OPTION_RESULT' => $point,
					'OS_OPTION_IMG' => $os_graphic_img,
		...
		foreach( $agent_point as $index => $point )
		{
			...
			$b_graphic = ( $b_graphic < $b_graphic_max - 1 ) ? $b_graphic + 1 : 0;
			// MX
			$pafiledb_template->assign_block_vars( "rating_b", array( 'B_IMG' => "pafiledb/images/stats/" . $agent_image[$index],
					'B_NAME' => $agent_lang[$index],
					'B_OPTION_RESULT' => $point,
					'B_OPTION_IMG' => $b_graphic_img,
		...
		$pafiledb_template->assign_vars( array( 'S_ACTION_CHART' => append_sid( 'dload.' . $phpEx . '?action=stats' ),
