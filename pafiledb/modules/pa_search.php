	...
	function main( $action )
	{
		...
		if ( isset( $_POST['submit'] ) || $search_author != '' || $search_keywords != '' || $search_id )
		{
			...
			if ( $search_results != '' )
			{
				...
				$pafiledb_template->assign_vars( array( 'L_SEARCH_MATCHES' => $l_search_matches ) 
					);
				...
				for( $i = 0; $i < count( $searchset ); $i++ )
				{
					...
					$pafiledb_template->assign_block_vars( 'searchresults', array( 'CAT_NAME' => $searchset[$i]['cat_name'],
				...
				$pafiledb_template->assign_vars( array( 'PAGINATION' => generate_pagination( $base_url, $total_match_count, $pafiledb_config['settings_file_page'], $start ),
		...
		if ( !isset( $_POST['submit'] ) || ( $search_author == '' && $search_keywords == '' && !$search_id ) )
		{
			...
			$pafiledb_template->assign_vars( array( 'S_SEARCH_ACTION' => append_sid( 'dload.php' ),
					'S_CAT_MENU' => $dropmenu,
