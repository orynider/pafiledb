...
class pafiledb_toplist extends pafiledb_public
{
	function main( $action )
	{
		...
		$pafiledb_template->assign_vars( array( 'DOWNLOAD' => $pafiledb_config['settings_dbname'],
		...
		switch ( $mode )
		{
			case 'newest': 
				...
				$pafiledb_template->assign_vars( array( 'IS_NEWEST' => true,
				...
				if ( empty( $selected_date ) )
				{
					for( $j = 0; $j <= $days - 1; $j++ )
					{
						...
						$pafiledb_template->assign_block_vars( 'files_date', array( 'U_DATES' => append_sid( 'dload.' . $phpEx . '?action=toplist&mode=newest&days=7&selected_date=' . $day_time ),
				...
				{
					$pafiledb_template->assign_vars( array( 'FILE_LIST' => true,
			...
			case 'rating':
				...
				$pafiledb_template->assign_vars( array( 'IS_POPULAR' => true,
						'FILE_LIST' => true,
				...
				for( $i = 0; $i < count( $searchset ); $i++ )
				{
					...
					$pafiledb_template->assign_block_vars( 'files_row', array( 'CAT_NAME' => $searchset[$i]['cat_name'],
							'FILE_NEW_IMAGE' => $images['pa_file_new'],
							'PIN_IMAGE' => $posticon,
